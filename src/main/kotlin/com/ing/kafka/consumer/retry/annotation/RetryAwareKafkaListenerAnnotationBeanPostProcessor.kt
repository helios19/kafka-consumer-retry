...
import com.ing.kafka.consumer.retry.listener.KafkaRetryPolicyErrorHandler
...
open class RetryAwareKafkaListenerAnnotationBeanPostProcessor<K, V>(
    ...
    private fun retryPolicyFor(bean: Any, method: Method): RetryPolicy? =
            AnnotationUtils.findAnnotation(method as AnnotatedElement, RetryPolicy::class.java) ?: retryPolicyForRetryKafkaListener(bean, method)

    private fun retryPolicyForRetryKafkaListener(bean: Any, method: Method): RetryPolicy? =
            AnnotationUtils.findAnnotation(method as AnnotatedElement, RetryKafkaListener::class.java)?.let { retryPolicyFor(it.retryPolicyId, bean) }
